...
func serve(args []string)error{
	...
	srv:=&http.Server{Addr:listenAddr,Handler:panel.Handler(),ReadHeaderTimeout:5*time.Second,ReadTimeout:20*time.Second,WriteTimeout:2*time.Minute,IdleTimeout:60*time.Second,MaxHeaderBytes:1<<20};log.Printf("X-port %s listening on %s",version,listenAddr);err=srv.ListenAndServe();if errors.Is(err,http.ErrServerClosed){return nil};return err
}
...
func initDB(args []string)error{fs:=flag.NewFlagSet("init",flag.ContinueOnError);data:=fs.String("data","/etc/x-port","data directory");user:=fs.String("admin-user","admin","admin username");listen:=fs.String("listen","127.0.0.1:8080","initial panel listen address");if err:=fs.Parse(args);err!=nil{return err};reader:=bufio.NewReader(os.Stdin);password,err:=reader.ReadString('\n');if err!=nil&&!errors.Is(err,io.EOF){return fmt.Errorf("read password from stdin: %w",err)};password=strings.TrimRight(password,"\r\n");if len(password)<12{return errors.New("admin password must be at least 12 characters")};st,err:=openData(*data);if err!=nil{return err};defer st.Close();hash,err:=bcrypt.GenerateFromPassword([]byte(password),bcrypt.DefaultCost);if err!=nil{return err};if err:=st.SetAdmin(*user,string(hash));err!=nil{return err};if err:=st.SetSetting("panel_listen",strings.TrimSpace(*listen));err!=nil{return err};_ = st.SetSetting("xray_api_port","10085");_ = st.SetSetting("port_min","20000");_ = st.SetSetting("port_max","60000");return nil}
...
func migrateDB(args []string)error{
	...
	adminNames:=make([]string,0,len(result.Admins));for _,a:=range result.Admins{adminNames=append(adminNames,a.Username)};enc:=json.NewEncoder(os.Stdout);enc.SetIndent("","  ");_ = enc.Encode(map[string]any{"source":*from,"compatible":len(result.Accounts),"adminUsernames":adminNames,"panelListen":result.PanelListen,"globalXrayConfig":globalConfig!="","warnings":result.Warnings,"skipped":result.Skipped,"apply":*apply})
	if !*apply{return nil};if len(result.Skipped)>0{return fmt.Errorf("refusing apply: %d source inbound(s) require attention",len(result.Skipped))};st,err:=openData(*data);if err!=nil{return err};defer st.Close();if err:=st.ReplaceAccounts(result.Accounts);err!=nil{return err};if len(result.Admins)>0{if err:=st.ReplaceAdmins(result.Admins);err!=nil{return err}};if result.PanelListen!=""{if err:=st.SetSetting("panel_listen",result.PanelListen);err!=nil{return err}};if globalConfig!=""{if err:=st.SetSetting("xray_global_config",globalConfig);err!=nil{return err}};return nil
}

...
func serve(args []string)error{
	...
	listenAddr:=strings.TrimSpace(*listen);if listenAddr==""{listenAddr=stringSetting(st,"panel_listen","")};if listenAddr==""{listenAddr=defaults.PanelListen}
	apiPort:=*apiPortFlag;if apiPort==0{apiPort=intSetting(st,"xray_api_port",defaults.XrayAPIPort)};if apiPort<1||apiPort>65535{return fmt.Errorf("invalid Xray API port %d",apiPort)}
	...
	basePath:=panelpath.Normalize(stringSetting(st,"panel_base_path",defaults.PanelBasePath))
...
func runtimeLoop(ctx context.Context,accounts *service.Accounts,stats *xray.StatsReader){ticker:=time.NewTicker(10*time.Second);defer ticker.Stop();lastLogged:=time.Time{};tick:=func(){now:=time.Now();values,err:=stats.ReadAndReset(ctx);if err!=nil{_ = accounts.ApplyTraffic(nil,now);if time.Since(lastLogged)>=time.Minute{log.Printf("Xray stats unavailable: %v",err);lastLogged=time.Now()};return};if err:=accounts.ApplyTraffic(values,now);err!=nil&&time.Since(lastLogged)>=time.Minute{log.Printf("account runtime policy failed: %v",err);lastLogged=time.Now()}};tick();for{select{case<-ctx.Done():return;case<-ticker.C:tick()}}}

func initDB(args []string)error{fs:=flag.NewFlagSet("init",flag.ContinueOnError);data:=fs.String("data","/etc/x-port","data directory");user:=fs.String("admin-user","admin","admin username");listen:=fs.String("listen",defaults.PanelListen,"initial panel listen address");if err:=fs.Parse(args);err!=nil{return err};reader:=bufio.NewReader(os.Stdin);password,err:=reader.ReadString('\n');if err!=nil&&!errors.Is(err,io.EOF){return fmt.Errorf("read password from stdin: %w",err)};password=strings.TrimRight(password,"\r\n");if len(password)<12{return errors.New("admin password must be at least 12 characters")};st,err:=openData(*data);if err!=nil{return err};defer st.Close();hash,err:=bcrypt.GenerateFromPassword([]byte(password),bcrypt.DefaultCost);if err!=nil{return err};if err:=st.SetAdmin(*user,string(hash));err!=nil{return err};if err:=st.SetSetting("panel_listen",strings.TrimSpace(*listen));err!=nil{return err};_ = st.SetSetting("panel_base_path",defaults.PanelBasePath);_ = st.SetSetting("panel_cert_file","");_ = st.SetSetting("panel_key_file","");_ = st.SetSetting("panel_domain","");_ = st.SetSetting("xray_api_port",strconv.Itoa(defaults.XrayAPIPort));_ = st.SetSetting("port_min",strconv.Itoa(defaults.PortMin));_ = st.SetSetting("port_max",strconv.Itoa(defaults.PortMax));return nil}
...
func normalizeBasePath(v string)string{return panelpath.Normalize(v)}

...
func main() {
	...
	var err error
	switch os.Args[1] {
	case "serve": err = serve(os.Args[2:])
	case "init": err = initDB(os.Args[2:])
	case "migrate": err = migrateDB(os.Args[2:])
	case "render": err = render(os.Args[2:])
	case "xray-check": err = xrayCheck(os.Args[2:])
	case "xray-update": err = xrayUpdate(os.Args[2:])
	case "version": fmt.Println(version); return
...
func openData(data string) (*store.Store, error) {
	if err := os.MkdirAll(data, 0700); err != nil { return nil, err }
...
func serve(args []string) error {
	...
	apiPortFlag := fs.Int("xray-api-port", 0, "local Xray API port override")
	if err := fs.Parse(args); err != nil { return err }
	st, err := openData(*data); if err != nil { return err }; defer st.Close()

	listenAddr := strings.TrimSpace(*listen)
	if listenAddr == "" {
		if stored, ok, err := st.Setting("panel_listen"); err != nil { return err } else if ok && strings.TrimSpace(stored) != "" { listenAddr = strings.TrimSpace(stored) }
	}
	if listenAddr == "" { listenAddr = "127.0.0.1:8080" }
	apiPort := *apiPortFlag
	if apiPort == 0 { apiPort = intSetting(st, "xray_api_port", 10085) }
	if apiPort < 1 || apiPort > 65535 { return fmt.Errorf("invalid Xray API port %d", apiPort) }

	static, err := webui.FS(); if err != nil { return err }
	...
	panel := server.New(st, accounts, updater, static).WithRuntime(server.RuntimeOptions{DataDir:*data, XrayService:*xrayService, PanelService:"xport.service", Manager:manager})

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	stats := &xray.StatsReader{BinaryPath:*xrayBin, Server:fmt.Sprintf("127.0.0.1:%d",apiPort), Timeout:4*time.Second}
	go runtimeLoop(ctx, accounts, stats)
...
func runtimeLoop(ctx context.Context, accounts *service.Accounts, stats *xray.StatsReader) {
	ticker := time.NewTicker(5*time.Second); defer ticker.Stop()
	lastLogged := time.Time{}
	tick := func() {
		now := time.Now()
		values, err := stats.ReadAndReset(ctx)
		if err != nil {
			// Expiry and monthly reset still need to run while Xray is temporarily down.
			_ = accounts.ApplyTraffic(nil, now)
			if time.Since(lastLogged) >= time.Minute { log.Printf("Xray stats unavailable: %v", err); lastLogged=time.Now() }
			return
		}
		if err := accounts.ApplyTraffic(values, now); err != nil && time.Since(lastLogged) >= time.Minute { log.Printf("account runtime policy failed: %v", err); lastLogged=time.Now() }
	}
	tick()
	for {
		select { case <-ctx.Done(): return; case <-ticker.C: tick() }
	}
}
...
func initDB(args []string) error {
	...
	password, err := reader.ReadString('\n'); if err != nil && !errors.Is(err, io.EOF) { return fmt.Errorf("read password from stdin: %w", err) }
	password = strings.TrimRight(password, "\r\n")
	if len(password) < 12 { return errors.New("admin password must be at least 12 characters") }
	st, err := openData(*data); if err != nil { return err }; defer st.Close()
	hash, err := bcrypt.GenerateFromPassword([]byte(password), bcrypt.DefaultCost); if err != nil { return err }
	if err := st.SetAdmin(*user, string(hash)); err != nil { return err }
	if err := st.SetSetting("panel_listen", strings.TrimSpace(*listen)); err != nil { return err }
	_ = st.SetSetting("xray_api_port", "10085")
	_ = st.SetSetting("port_min", "20000")
	_ = st.SetSetting("port_max", "60000")
	return nil
}
...
func migrateDB(args []string) error {
	...
	if err := fs.Parse(args); err != nil { return err }
	result, err := migrate.ReadXUI(*from); if err != nil { return err }
	adminNames := make([]string,0,len(result.Admins)); for _,a:=range result.Admins{adminNames=append(adminNames,a.Username)}
	enc:=json.NewEncoder(os.Stdout);enc.SetIndent("","  ");_ = enc.Encode(map[string]any{"source":*from,"compatible":len(result.Accounts),"adminUsernames":adminNames,"panelListen":result.PanelListen,"warnings":result.Warnings,"skipped":result.Skipped,"apply":*apply})
	if !*apply{return nil};if len(result.Skipped)>0{return fmt.Errorf("refusing apply: %d source inbound(s) require attention",len(result.Skipped))}
	st,err:=openData(*data);if err!=nil{return err};defer st.Close()
	if err:=st.ReplaceAccounts(result.Accounts);err!=nil{return err}
	if len(result.Admins)>0{if err:=st.ReplaceAdmins(result.Admins);err!=nil{return err}}
	if result.PanelListen!=""{if err:=st.SetSetting("panel_listen",result.PanelListen);err!=nil{return err}}
	return nil
}

func render(args []string) error {
	fs:=flag.NewFlagSet("render",flag.ContinueOnError);data:=fs.String("data","/etc/x-port","data directory");output:=fs.String("output","","output config path");apiPortFlag:=fs.Int("api-port",0,"local Xray API port override")
	if err:=fs.Parse(args);err!=nil{return err};if *output==""{return errors.New("--output is required")}
	st,err:=openData(*data);if err!=nil{return err};defer st.Close();accounts,err:=st.Accounts();if err!=nil{return err};apiPort:=*apiPortFlag;if apiPort==0{apiPort=intSetting(st,"xray_api_port",10085)}
	cfg,err:=xray.BuildConfig(accounts,apiPort);if err!=nil{return err};if err:=os.MkdirAll(filepath.Dir(*output),0750);err!=nil{return err};tmp:=*output+".tmp";if err:=os.WriteFile(tmp,cfg,0640);err!=nil{return err};return os.Rename(tmp,*output)
}

...
func TestMonthlyResetReenablesOnlyQuotaSuspendedAccount(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := st.ReplaceAccounts([]modelAccountAlias{a}); err != nil { t.Fatal(err) }
	fa := &fakeApply{}
...
// Alias keeps the test literal concise while still exercising the real model type.
type modelAccountAlias = model.Account

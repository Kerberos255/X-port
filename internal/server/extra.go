...
func safeExportName(name string) string {
	name = strings.Trim(exportNameRE.ReplaceAllString(strings.TrimSpace(name), "-"), "-._")
	if name == "" { return "account" }
	if len(name) > 64 { name = name[:64] }
	return name
}
...
func (s *Server) checkGeodata(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	info, _, err := s.updater.CheckGeodata(ctx)
...
func (s *Server) updateGeodata(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	info, err := s.updater.UpdateGeodata(ctx)
	if err != nil { writeError(w, 502, err.Error()); return }
	writeJSON(w, 200, info)
}
...
func (s *Server) importBackup(w http.ResponseWriter, r *http.Request) {
	dir := s.backupDir()
	if dir == "" { writeError(w, 503, "backup directory is not configured"); return }
	...
	if err != nil { writeError(w, 500, err.Error()); return }
	n, copyErr := io.Copy(f, io.LimitReader(http.MaxBytesReader(w, r.Body, 64<<20+1), 64<<20+1))
	closeErr := f.Close()
	if copyErr != nil || closeErr != nil || n > 64<<20 {
		...
		if copyErr != nil { writeError(w, 400, copyErr.Error()); return }
		writeError(w, 500, closeErr.Error()); return
	}
	if err := os.Rename(tmp, path); err != nil { _ = os.Remove(tmp); writeError(w, 500, err.Error()); return }
	if _, err := backup.Load(dir, name); err != nil {
		_ = os.Remove(path)
		writeError(w, 400, "invalid X-port backup: "+err.Error())
		return
	}
	st, err := os.Stat(path)
	if err != nil { writeError(w, 500, err.Error()); return }

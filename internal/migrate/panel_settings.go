...
func readPanelSettings(db *sql.DB) (PanelSettings, []string, error) {
	...
	if err != nil {
		return PanelSettings{}, nil, err
	}
	if !exists {
		return PanelSettings{}, nil, nil
	}
	rows, err := db.Query(`SELECT key,value FROM settings WHERE key IN ('webBasePath','webCertFile','webKeyFile','webDomain')`)
	if err != nil {
		return PanelSettings{}, nil, fmt.Errorf("read x-ui panel TLS settings: %w", err)
	}
	defer rows.Close()
	...
	out := PanelSettings{
		BasePath: panelpath.Normalize(values["webBasePath"]),

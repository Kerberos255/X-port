...
install -d -m 0750 "$DATA_DIR" "$DATA_DIR/xray" "$DATA_DIR/backups" "$PREFIX/bin" "$LIB_DIR"
install -m 0755 "$LOCAL_BIN" "$XPORT_BIN"
install -m 0755 "$ROOT/scripts/migrate-xpanel.sh" "$LIB_DIR/migrate-xpanel.sh"
...
if systemctl is-active --quiet x-ui.service 2>/dev/null || [[ -f /etc/x-ui/x-ui.db ]]; then
  ...
  echo "Next: sudo xport  (choose X-Panel / 3x-ui migration)"
else
...
echo "Management menu: sudo xport"
